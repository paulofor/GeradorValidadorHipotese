...
public abstract class Adapter {
	...
	/**
	 * A callback receiving the HTTP response body in the binary form.
	 */
	public interface BinaryCallback {
		 ...
		 *            The response body.
		 * @param contentType
		 *            The value of "Content-Type" response header.
		 */
	...
	/**
	 * A callback that parses a JSON response. If the return value is not valid
	 * JSON, then the {@link #onError(Throwable)} method is called.
	 */
	public abstract static class JsonCallback implements Callback {

		/**
		 * The method invoked when the call completes successfully. The response
		 * is a {@link org.json.JSONObject}, {@link org.json.JSONArray},
		 * String, Number, Boolean, or {@link org.json.JSONObject#NULL}.
		 * 
		 * @param response
		 *            The JSON object.
		 */
	...
	/**
	 * A callback that returns a JSON object. If the return value is not a JSON
	 * object or "null", then the {@link #onError(Throwable)} method is called.
	 */
	public abstract static class JsonObjectCallback extends JsonCallback {
		...
		public void onSuccess(Object response) {
			if (response instanceof JSONObject) {
				...
				onError(new JSONException("Expecting a JSON object: "
						+ response));
			}
		}
	}
	...
	/**
	 * A callback that returns a JSON array. If the return value is not a JSON
	 * array or "null", then the {@link #onError(Throwable)} method is called.
	 */
	public abstract static class JsonArrayCallback extends JsonCallback {
		...
		public void onSuccess(Object response) {
			if (response instanceof JSONArray) {
				...
				onError(new JSONException("Expecting a JSON array: " + response));
			}
		}
	}
	 ...
	 * 
	 * @param method
	 *            The method to invoke, e.g. <code>"module.doSomething"</code>.
	 * @param parameters
	 *            The parameters to invoke with.
	 * @param callback
	 *            The callback to invoke when the execution finishes.
	 */
	public abstract void invokeStaticMethod(String method,
			Map<String, ? extends Object> parameters, Callback callback);

	/**
	 * Invokes a remotable method exposed statically on the server, parses the
	 * response as binary data.
	 * <p>
	 * Unlike {@link #invokeInstanceMethod(String, Map, Map, BinaryCallback)},
	...
	public void invokeStaticMethod(String method,
			Map<String, ? extends Object> parameters, BinaryCallback callback) {
		throw new UnsupportedOperationException(getClass().getName()
				+ " does not support binary responses.");
	}
	 ...
	 * or creates an object with the appropriate type. Then and only then is the
	 * method invoked on that object. The two parameter dictionaries correspond
	 * to these two steps: `creationParameters` for the former, and `parameters`
	 * for the latter.
	 * 
	 * @param method
	 ...
	 *            The parameters the virtual object should be created with.
	 * @param parameters
	 *            The parameters to invoke with.
	 * @param callback
	 *            The callback to invoke when the execution finishes.
